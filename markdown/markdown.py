...
class Markdown:
    ...
    def __header_tag(self):
        ...
        for i in range(6, 0, -1):
            ...
            self.markdown = re.sub(r'^{} (.*?$)'.format('#' * i),
                                   r'<h{0}>\1</h{0}>'.format(i), self.markdown, flags=re.M)

    def __li_tag(self):

        self.markdown = re.sub(
            r'^\* (.*?$)', r'<li>\1</li>', self.markdown, flags=re.M)

    def __ul_tag(self):

        self.markdown = re.sub(
            r'(<li>.*</li>)', r'<ul>\1</ul>', self.markdown, flags=re.S)

    def __p_tag(self):

        self.markdown = re.sub(
            r'^(?!<[hlu])(.*?$)', r'<p>\1</p>', self.markdown, flags=re.M)
